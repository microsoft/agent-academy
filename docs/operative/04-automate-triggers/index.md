...
Previously you learned about topic triggers in [Recruit](../../recruit/07-add-new-topic-with-trigger/index.md), however you might still be wondering how _Topic_ triggers differ from _Event_ triggers, and why that distinction matters for understanding what makes an agent autonomous.
...
1. We'll now see the **Trigger name** and the **Sign in** connection references for the apps listed. Rename the trigger name to the following:
    ...
    ![Update details for trigger name and check connection references](assets/03_triggerName.png)
...
1. The final step is to set the input properties of the trigger. Update the following properties to the following,
    ...
    ![Configure trigger inputs](assets/03_triggerSettings.png)
...
1. We're now going to update the event trigger to add some more automation capabilities. Select the **ellipsis (...)** by the trigger and select **Edit in Power Automate**.
    ...
    ![Select Edit in Power Automate](assets/03_editInPowerAutomate.png)
...
1. We still need to configure several more parameters, select **Show all** and in the **Cover Letter** field, select the **fx icon** to the right.
    ...
    In the **Function tab**, enter the following expression that uses the same expression in the previous [mission](../03-multi-agent/index.md).
...
1. We've completed configuring this action and our agent flow 🙌🏻 You're doing great! Now let's save our event trigger flow by selecting **Save**.
    ...
    ![Save](assets/03_saveTriggerFlow.png)
...
1. In the **Hiring Agent** select the **Agents** tab and select the **Application Intake Agent**
    ...
    ![Select Application Intake Agent](assets/03_selectAppIntakeAgent.png)

1. Scroll down to **Tools** and select **+ Add**.

    ![Add Tool](assets/03_addToolHiring.png)

1. The **Add tool** modal will appear. Select **+ New tool**.

    ![Select New Tool](assets/03_newToolbtn.png)

1. Select **Agent flow**.

    ![Select Agent flow](assets/03_agentflowselect.png)
