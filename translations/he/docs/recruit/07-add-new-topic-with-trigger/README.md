...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-10-17T06:03:24+00:00",
...
יצרתם סוכן. הוא מקשיב, לומד ועונה על שאלות - אבל עכשיו הגיע הזמן להיות יותר טקטיים. במשימה הזו, תעמיקו ותלמדו את הסוכן שלכם איך להגיב בצורה מדויקת להנחיות ספציפיות.

עם נושאים וטריגרים, הסוכן שלכם יכול:

- לזהות כוונה

- לנתב שיחות עם לוגיקה
...
1. להבין מה הם נושאים ותפקידם ביצירת שיחות מובנות עבור הסוכן שלכם  
...
ישנן שלוש מטרות נפוצות לנושאים בהתבסס על מה שהמשתמשים צריכים:

**מידע** - עונה על שאלות כגון:
...
ניתן גם ליצור נושאים לשאלות עמומות כמו `אני צריך עזרה`, שמבקשות מהמשתמשים פרטים נוספים לפני שממשיכים.
...
1. **נושאים מערכתיים** - אלו מובנים ומטפלים באירועים נפוצים כגון:  
    - התחלת שיחה  
    - סיום שיחה  
    - טיפול בשגיאות  
...
1. **נושאים מותאמים אישית** - אתם יוצרים אותם כדי לטפל במשימות או שאלות ספציפיות כגון:  
    - בקשת חופשה לעובד  
...
- `אני צריך מכשיר חדש`  
- `האם אני יכול לבקש מכשיר?`  
- `האם תוכל לעזור לי בבקשת מכשיר?`  

### 💬 צמתים בשיחה

נושא מורכב מצמתים שהם שלבים שהסוכן עוקב אחריהם לאחר שהנושא מופעל. אתם מחברים את השלבים הללו כדי לבנות זרימת שיחה שהסוכן שלכם עוקב אחריה כשהוא מתקשר עם משתמשים.

חשבו עליהם כהוראות או פעולות כגון:
...
הבאים הם סוגי הצמתים העיקריים שניתן להוסיף לסוכן:
...
ניתן להתאים אישית הודעות באמצעות משתנים, להוסיף מספר גרסאות של הודעות לגיוון, ואפילו להתאים את הפלט הקולי לערוצים המופעלים בקול.
...
צומת זה משמש לשאול את המשתמשים מידע ספציפי במהלך השיחה ולשמור את תשובותיהם במשתנים לשימוש מאוחר יותר.

ניתן להתאים את סוג השאלה כמו קלט טקסט או להשתמש בישויות לרשימה מוגדרת של ערכים שהמשתמש בוחר מהם, ולהגדיר כיצד הסוכן צריך להתנהג אם המשתמש נותן תשובה לא תקינה או מדלג על השאלה.

הוא גם תומך בתוכן עשיר כמו תמונות ותשובות מהירות, ומאפשר לכם לכוונן הגדרות אימות, חזרה על השאלה והפרעה כדי לגרום לשיחה לזרום בצורה חלקה.

!!! tip  
    חשבו על זה כעל בלוק "שאל והקשב" שעוזר לסוכן שלכם לתקשר עם משתמשים בצורה מובנית שאתם מגדירים.

#### שאל עם כרטיס אדפטיבי

- **מטרה** - שולח כרטיס עשיר ואינטראקטיבי באמצעות JSON.  
- **דוגמה** - כרטיס שמציג בוחר תאריך בלוח שנה למשתמש לבחור תאריך.  

צומת זה מציג כרטיסים עשירים ואינטראקטיביים שהמשתמשים יכולים למלא ולהגיש כמו טפסים עם תיבות טקסט, כפתורים ותמונות. הוא לוכד את הקלט של המשתמש ושומר אותו במשתנים, שהסוכן שלכם יכול להשתמש בהם מאוחר יותר בשיחה.

!!! tip  
    חשבו על זה כעל בלוק "בונה טפסים" מותאם אישית שעושה את הסוכן שלכם יותר אינטראקטיבי ומסוגל לאסוף מידע מפורט ממשתמשים.

#### הוסף תנאי

- **מטרה** - מוסיף לוגיקה לשיחה. הוא בודק משהו ומחליט מה לעשות הלאה.  
...
- **מטרה** - שומר או מנקה מידע (שנקרא משתנים) במהלך השיחה.  
...
!!! tip  
    חשבו על זה כעל "קופסת זיכרון" שעוזרת לסוכן שלכם לזכור מידע ולהשתמש בו כשהשיחה ממשיכה עם המשתמש.
...
צומת זה מאפשר לסוכן שלכם לקפוץ מנושא אחד לאחר מבלי להתחיל מחדש את השיחה, לסיים את הנושא, להעביר או לסיים את השיחה, או לעבור לשלב אחר בתוך אותו נושא. הוא עוזר להנחות משתמשים דרך חלקים שונים בזרימת השיחה על ידי מעבר חלק בין נושאים, וניתן להעביר משתנים ביניהם כדי לשמור על הקשר.
...
- **מטרה** - משתמש במודל שפה גדול כדי ליצור תגובות טבעיות ודמויות אדם בהתבסס על שאלת המשתמש וכל נתונים מחוברים.  
- **דוגמה** - משתמש במקור הידע המחובר שמכיל מידע על זכויות חופשה כדי לענות על שאלות המשתמש בנוגע לבקשות חופשה.  
...
!!! tip  
    חשבו על זה כעל "בלוק תשובה חכמה" שעוזר לסוכן שלכם לתת תגובות מועילות ומדויקות על ידי חיפוש תוכן אמין שאתם מגדירים.
...
!!! tip  
    חשבו על זה כעל "הושיט יד וקבל מידע" שעוזר לסוכן שלכם לדבר עם שירותים אחרים כמו אחזור פרטי משתמש או שליחת נתונים למערכת אחרת.

#### שלח אירוע

- **מטרה** - מאפשר לסוכן שלכם לשלוח פעולות שאינן הודעות, כגון עדכוני מערכת או טריגרים לכלים - ללקוח או לערוץ, ועוזר לו לבצע משימות.  
- **דוגמה** - תגובה למשתמש שמצטרף לצ'אט על ידי הצגת הודעת ברכה.  

צומת זה מאפשר לסוכן שלכם לשלוח פעולות שאינן הודעות למערכות חיצוניות או ערוצים, שיכולים אז להחליט כיצד להגיב. אתם נותנים לכל אירוע שם ומצמידים ערך, שיכול להיות מספר פשוט או טקסט, משתנה או נוסחת Power Fx, והוא נשלח כאובייקט JSON.

!!! tip  
    חשבו על זה כעל "טריגר שקט" שעוזר לסוכן שלכם לעשות דברים מאחורי הקלעים או לתקשר עם כלים חיצוניים מבלי שהמשתמש יצטרך לומר משהו.

## 🏋🏻‍♀️ שימוש ב-Power Fx בצמתים שלכם

ב-Copilot Studio, Power Fx הוא שפת תכנות בקוד נמוך המשמשת להוספת לוגיקה והתנהגות דינמית לסוכן שלכם. זו אותה שפה שמשתמשים בה ב-Microsoft Power Apps, והיא נועדה להיות פשוטה ודמוית Excel, מה שהופך אותה לקלה למפתחים ולא-מפתחים.
...
- **עוצמתי:** מאפשר לכם להתאים אישית שיחות, לאמת קלט ולשלוט כיצד הסוכן שלכם מתנהג בהתבסס על נתוני משתמשים.  
...
![הוסף נושא](../../../../../translated_images/7.0_04_AddATopic.111df124a4a7ff8b41e3f577fbef08885ac52d9d6c0c705a82f5968e5ccc384d.he.png)

#### למה זה שימושי

- זה נותן לכם שליטה מלאה על איך הסוכן שלכם מגיב.  
- ניתן להתאים לוגיקה באמצעות משתנים, Power Fx ותנאים.  
- זה מושלם לבניית חוויות מותאמות אישית לצרכים עסקיים ספציפיים.  

### 2. יצירה עם Copilot
זה מאפשר לך לתאר מה אתה רוצה באמצעות שפה טבעית, ו-Copilot יבנה עבורך את השיחה. אותו דבר חל גם על עריכת הנושא שלך, השתמש בשפה טבעית ו-Copilot יבדוק ויערוך את הנושא עבורך.
...
- מאיץ את הפיתוח בעזרת סיוע AI.
- מאפשר לך להתמקד בלוגיקה ובחוויית המשתמש במקום בהגדרות חוזרות.
- מקל על שיפור זרימות השיחה עם מינימום מאמץ.
...
- **עריכת נושא**
      ...
      - `סכם את המידע שנאסף בכרטיס אדפטיבי.`

## 👩🏻‍🎨 אוקיי, איך אני מעצב נושאים לסוכן שלי?

### 🧙🏻‍♂️ שלב 1 - הבן מה המשתמשים צריכים

התחל בזיהוי שאלות או משימות נפוצות שהמשתמשים יבקשו מהסוכן שלך. אלו יכולים להיות:

- שאלות שהמשתמשים שואלים לעיתים קרובות כמו, `מה הזכאות שלי לימי מחלה?`
- משימות נפוצות שהמשתמשים רוצים להשלים כמו הגשת טופס
- בעיות שהמשתמשים נתקלים בהן לעיתים קרובות כמו בעיות התחברות

### 📦 שלב 2 - קבץ את התרחישים

ארגן את צרכי המשתמשים לשלוש קטגוריות בהתבסס על מה שלמדנו קודם - מטרת הנושא:

- מידע - המשתמש רוצה לדעת משהו
- השלמת משימה - המשתמש רוצה לעשות משהו
- פתרון בעיות - המשתמש זקוק לעזרה בפתרון בעיה

### 🗺️ שלב 3 - מפה את השיחה
...
!!! tip
    שמור את השיחה קצרה וממוקדת. שאל רק את מה שצריך.
...
- סוכן: `בטח! מאיזה תאריך תרצה להתחיל את החופשה?`
...
- סוכן: `תודה על הפרטים. הגשתי את בקשת החופשה שלך מ-15 ביולי עד 22 ביולי לחופשה משפחתית. תקבל אישור בקרוב.`

### 🤖 שלב 5 - השתמש ב-AI לשאלות בלתי צפויות

גם אם עיצבת נושא לבקשות חופשה, ייתכן שמשתמשים ישאלו שאלות שאינן מכוסות ישירות. כאן נכנסות לתמונה תכונות AI כמו נושא מערכת _Conversational boosting_.

נושא זה כולל צומת תשובות גנרטיביות, שמאפשר לסוכן שלך להתחיל להשתמש במקורות מידע מחוברים מיד. כל מקורות המידע שנוספו ברמת הסוכן נכללים אוטומטית בצומת התשובות הגנרטיביות בתוך נושא מערכת _Conversational boosting_.
...
- אין צורך ליצור נושא ידני לכל שאלה הקשורה למדיניות.
- AI יכול לשלוף תשובות מדויקות ממקורות מידע אמינים.
- זה משפר את חוויית המשתמש בכך שהסוכן מרגיש חכם ומגיב יותר.
...
`בטח! מאיזה תאריכים תרצה לקחת חופשה?`

כרטיס אדפטיבי כולל בוחר תאריכים לתחילת וסיום.

#### שלב 3: המשתמש מספק תאריכים

המשתמש בוחר תאריך התחלה כ-5 באוגוסט 2025 ותאריך סיום כ-10 באוגוסט 2025, ומגיש את הכרטיס. ערכי התאריכים נשמרים בפלט של הכרטיס האדפטיבי כמשתנים.

#### שלב 4: זרימת ענן מבוצעת

זרימת ענן של Power Automate מבוצעת, יוצרת בקשה חדשה במערכת ניהול החופשות ושולחת דוא"ל להודיע למנהל על בקשת החופשה.

#### שלב 5: שלח הודעת אישור למשתמש

הסוכן מגיב עם,

`בקשת החופשה שלך מ-5 באוגוסט עד 10 באוגוסט הוגשה. המנהל שלך יבדוק ויחזור אליך בקרוב.`

## 🧪 מעבדה 07 - הוסף נושא חדש עם צמתים לשיחה
...
- [7.2 הגדר את הקלטים והפלטים של המפעיל](../../../../../docs/recruit/07-add-new-topic-with-trigger)
...
1. **סוכן עזרה של Contoso**
    ...
    נשתמש באותו סוכן שנוצר קודם ב-[שיעור 06 - יצירת סוכן מותאם אישית באמצעות שפה טבעית עם Copilot והצמדתו לנתונים שלך](../06-create-agent-from-conversation/README.md).
...
1. לשונית **נושאים** תיטען ובברירת מחדל יוצגו הנושאים _מותאמים אישית_. ניתן לסנן נושאים לפי הכל, מותאם אישית ומערכת. הנושאים המותאמים אישית והמערכת שאתה רואה כרגע נוצרו אוטומטית כאשר הסוכן הוקצה.
...
### 7.2 הגדר את הקלטים והפלטים של המפעיל

1. כעת, נוסיף משתנה קלט חדש ש-AI הגנרטיבי ישתמש בו בתזמור שלו כדי לחלץ את סוג המכשיר מהודעת המשתמש. בחר את **שלוש הנקודות (...)** בנושא ובחר **פרטים** כדי לצפות בלוח פרטי הנושא.
...
1. לוח **פרטי הנושא** נטען כעת. בחר את לשונית **קלט**.
...
1. כעת עלינו להגדיר את משתני הקלט והפלט שלנו. להלן תכונות שניתן להגדיר עבור קלטים ופלטים של נושא.

    | שדה    | ערך |
    | ---------- | :--------- |
    | איך הסוכן ימלא את הקלט הזה? | קובע איך הסוכן ימלא את המשתנה הזה עם ערך לפני הפעלת הנושא. כברירת מחדל זה מוגדר ל_מילוי דינמי עם האפשרות הטובה ביותר_. אחרת ניתן לעקוף קלט עם ערך במקום לשאול את המשתמש|
    ...
    | תיאור    | התיאור עוזר לסוכן למלא קלטים באופן אוטומטי לפני הפעלת הנושא או ליצור שאלות כדי לבקש את הערכים   |

    ניתן להשאיר את _איך הסוכן ימלא את הקלט הזה?_, _סוג נתוני משתנה_ ו_שם תצוגה_ כפי שהם. עדכן את תכונת **זיהוי כ** ל-**תשובת המשתמש המלאה**.
...
1. כעת, נגדיר את הפלט עבור הנושא. בחר את לשונית **פלט**.
...
1. עדכן את התכונות הבאות.
    ...
    **תיאור משתנה** - העתק והדבק את הבא.
    ...
    ![תכונות פלט](../../../../../translated_images/7.2_09_OutputVariable.fb0e159fbad5052280040090352c50faf4d91228095c3762e75440b2842e0d29.he.png)

1. סיימנו להגדיר את הקלטים והפלטים של הנושא. בחר את **סמל X** כדי לצאת מלוח **פרטי הנושא**.

    ![צא מלוח פרטי הנושא.](../../../../../translated_images/7.2_10_ExitTopicDetailsPane.6e8981434f04049bef7ab93f9545ee433087e1c99cdfe27b355ee9858ddfde99.he.png)

### 7.3 הוסף כלי באמצעות מחבר

1. כעת נוסיף צומת שמאפשר לסוכן לשלוף את רשימת המכשירים מרשימת **Devices** של SharePoint. בחר את **סמל +** מתחת למפעיל.
...
1. יש ליצור חיבור חדש עבור המחבר. בחר את **סמל החץ** ובחר **צור חיבור חדש**.
...
1. חלונית הגדרת **קבל פריטים** תופיע ובברירת מחדל תראו את לשונית **קלטים**. בחרו בלשונית **התחלה** והכניסו תיאור בשדה **תיאור שימוש**. העתיקו והדביקו את הטקסט הבא.
...
1. כעת, כדי להציג רק מכשירים מהרשימה של SharePoint בהתבסס על
   ...
   עלינו להחיל מסנן. זה מתבצע על ידי הכנסת שאילתת מסנן בעזרת Power Fx. בחרו את **סמל שלוש הנקודות (...)**.
...
1. ביטוי Power Fx יוחל כעת בפרמטר קלט של שאילתת מסנן בפעולת **קבל פריטים**. לאחר מכן, בחרו בתצוגת **כל הפריטים** ב-**הגבלת עמודות לפי תצוגה**. זה ימשוך רק את העמודות ברשימה בהתבסס על התצוגה שנבחרה.
...
1. גללו מטה ובחלק **שימוש**, בחרו **גלובלי**. זה יהפוך את המשתנה לנגיש על ידי כל נושא.
...
1. בחרו את **סמל הפלוס +** כדי להוסיף צומת חדש, בחרו **ניהול משתנים** ולאחר מכן בחרו **הגדרת ערך משתנה**.
...
1. בחרו בסמל **גדול מ** עבור פרמטר הקלט של **הגדר משתנה**.
...
1. כעת נשתמש בביטוי PowerFx כדי להגדיר את ערך המשתנה כמאפיין `value` שהוחזר בתגובת **קבל פריטים**, ונגדיר את [היקף המשתנה](https://learn.microsoft.com/microsoft-copilot-studio/advanced-power-fx?WT.mc_id=power-172618-ebenitez) כגלובלי על ידי הוספת הקידומת `Global`.
...
הוראה זו תדריך את ה-AI הגנרטיבי להפעיל את הטריגר **מכשירים זמינים** כדי להציג את רשימת המכשירים הזמינים מרשימת SharePoint **Devices**. בחרו את כל משתנה הנושא בסוגריים מרובעים.
...
1. כעת נבדוק את הסוכן המעודכן שלנו. בחרו **בדיקה** בצד העליון הימני כדי להציג את חלונית הבדיקה ו**רעננו** את חלונית הבדיקה. הכניסו את ההודעה הבאה לסוכן.
...
1. לפני שהסוכן יכול להמשיך, המשתמש צריך לאשר שהחיבור שלו יכול לשמש. בחרו **אפשר**.

![בחרו אפשר](../../../../../translated_images/7.3_29_SelectAllow.9f508c4001270252924d889792ecf0cc2a984954b903bb00f7ce6b2dc43d08e3.he.png)

1. הסוכן יבצע את הכלי של SharePoint שמחזיר רשימה מסוננת של מכשירים שבהם סוג המכשיר שווה ל-"מחשב נייד" והסטטוס שווה ל"זמין", מתוך ביטוי Power Fx שהשתמשנו בו. תוחזר תגובה בפורמט של נקודות עבור המשתמש לקריאה.

![תוצאת בדיקה](../../../../../translated_images/7.3_30_TestResponse.b60253568edc8b68d737a62960f4a3fa3620d2ba4658b05aa2503ad5fd763383.he.png)

1. דבר אחרון ללמוד הוא צפייה בחיבורים בשימוש על ידי צפייה בדף _נהל את החיבורים שלך_ של הסוכן. בחרו את **סמל שלוש הנקודות (...)** ובחרו **נהל חיבור**.
...
1. דף זה הוא המקום שבו ניתן לראות את כל החיבורים שהסוכן משתמש בהם. כרגע, רק חיבור אחד מופיע שמקושר לכלי SharePoint שנוסף לנושא. בחרו **1 כלי** בעמודת **בשימוש על ידי**.

![בשימוש על ידי](../../../../../translated_images/7.3_32_UsedBy.1e5ff032f1e02a565a0dafdde4f9436486ed3f012fcc23b824871a71b6de543e.he.png)

1. כאן ניתן לראות את פרטי פעולת קבל פריטים וזוכרים את _תיאור השימוש_ שהכנסנו קודם? כאן נראה את תיאור השימוש. בחרו **סגור**.
...
1. חזרו ללשונית הדפדפן עם Copilot Studio ו**רעננו** את חלונית הבדיקה כדי לנקות את הבדיקה.

## ✅ משימה הושלמה

מזל טוב! 👏🏻 למדתם כיצד להוסיף נושא חדש מאפס, כיצד להוסיף כלי שמפעיל את פעולת מחבר SharePoint קבל פריטים ולהשתמש ב-Power Fx כדי לסנן את התגובה כך שתחזיר רק מכשירים שבהם הסטטוס שווה זמין וסוג המכשיר שווה מחשב נייד. 🙌🏻
...
🔗 [הגדרת נושאים לסוכן](https://learn.microsoft.com/microsoft-copilot-studio/guidance/defining-chatbot-topics?WT.mc_id=power-172618-ebenitez)
