...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-10-17T06:01:04+00:00",
...
Olet luonut agentin. Se kuuntelee, oppii ja vastaa kysymyksiin - mutta nyt on aika siirtyä taktisempiin toimiin. Tässä tehtävässä sukellat syvälle ja opetat agenttiasi vastaamaan tarkasti tiettyihin pyyntöihin.
...
Et vain rakenna dialogia, vaan kytket sen päätöksentekokorteksin. Tervetuloa Neuraaliseen Nexus-maailmaan.

## 🔎 Tavoitteet

Tässä tehtävässä opit:

1. Ymmärtämään, mitä aiheet ovat ja niiden roolin agenttisi rakenteellisessa keskustelussa
...
1. Luomaan räätälöityjä aiheita alusta alkaen käsittelemään tiettyjä käyttäjäpyyntöjä ja tehtäviä
1. Rakentamaan toimivan aiheen, joka yhdistyy SharePoint-dataan käyttäen liittimiä ja työkaluja
...
Aiheilla on kolme yleistä tarkoitusta käyttäjien tarpeiden perusteella:
...
- Tekemään keskusteluista luonnollisia.

- Ratkaisemaan käyttäjien ongelmat tehokkaasti.
...
1. **Järjestelmäaiheet** - nämä ovat sisäänrakennettuja ja käsittelevät yleisiä tapahtumia, kuten:
    ...
    - Virheiden käsittely
    - Käyttäjien pyytäminen kirjautumaan sisään
    - Keskustelun siirtäminen ihmiskäsittelijälle
...
Nämä ovat sanoja tai lauseita, joita käyttäjät saattavat sanoa aloittaakseen aiheen.

**Esimerkkejä:**

Lomapyyntöaiheelle laukaisulauseet voisivat olla:
...
Laitteen pyytämiseen liittyvälle aiheelle laukaisulauseet voisivat olla:
...
Aihe koostuu solmuista, jotka ovat askeleita, joita agentti seuraa, kun aihe laukaistaan. Näitä askeleita yhdistämällä rakennetaan keskustelun kulku, jota agentti seuraa vuorovaikutuksessa käyttäjien kanssa.

Ajattele näitä ohjeina tai toimina, kuten:
...
Seuraavat ovat agentille lisättävissä olevia pääsolmujen tyyppejä:
...
Voit personoida viestejä muuttujien avulla, lisätä useita viestivaihtoehtoja vaihtelun vuoksi ja jopa mukauttaa puheulostuloa ääniaktivoiduilla kanavilla.
...
Tätä solmua käytetään kysymään käyttäjiltä tiettyä tietoa keskustelun aikana ja tallentamaan heidän vastauksensa muuttujiksi myöhempää käyttöä varten.

Voit mukauttaa kysymyksen tyyppiä, kuten tekstisyötettä, tai käyttää entiteettejä määritellyn arvolistan kanssa, josta käyttäjä valitsee, ja määrittää, miten agentin tulisi toimia, jos käyttäjä antaa virheellisen vastauksen tai ohittaa kysymyksen.
...
- **Tarkoitus** - lähettää rikastetun, vuorovaikutteisen kortin JSON:n avulla.
- **Esimerkki** - kortti, joka näyttää kalenterin päivämäärävalitsimen käyttäjän valittavaksi.

Tämä solmu näyttää rikastettuja, vuorovaikutteisia kortteja, joita käyttäjät voivat täyttää ja lähettää, kuten lomakkeita, joissa on tekstikenttiä, painikkeita ja kuvia. Se tallentaa käyttäjän syötteen ja säilyttää sen muuttujissa, joita agenttisi voi käyttää myöhemmin keskustelussa.

!!! tip
    Ajattele tätä mukautettuna "lomakkeen rakentaja" -blokiksi, joka tekee agentistasi kiinnostavamman ja kykenevämmän keräämään yksityiskohtaista tietoa käyttäjiltä.
...
Tämä solmu luo päätöspisteitä agenttisi keskustelun kulkuun tarkistamalla, täyttääkö muuttuja tietyt kriteerit. Jos ehto on tosi tai epätosi, agentti seuraa eri polkuja.
...
!!! tip
    Ajattele tätä "muistilaatikkona", joka auttaa agenttiasi muistamaan tietoa ja käyttämään sitä keskustelun jatkuessa käyttäjän kanssa.
...
- **Esimerkki** - ohjaa "Lomakäytäntö" -aiheeseen.

Tämä solmu antaa agenttisi hypätä aiheesta toiseen ilman keskustelun uudelleenkäynnistämistä, lopettaa aiheen, siirtää tai lopettaa keskustelun tai siirtyä eri askeleeseen saman aiheen sisällä. Se auttaa ohjaamaan käyttäjiä keskustelun eri osien läpi siirtymällä sujuvasti aiheiden välillä, ja voit siirtää muuttujia niiden välillä säilyttääksesi kontekstin.
...
Tämä solmu antaa agenttisi kyvyn olla vuorovaikutuksessa ulkoisten järjestelmien kanssa tai suorittaa tiettyjä tehtäviä, kuten lähettää sähköposteja, tarkistaa säätä tai käyttää tietokantoja. Voit lisätä työkaluja sisäänrakennettujen liittimien, mukautettujen API:iden, agenttiprosessien, kehotteiden tai yhdistää Mallikontekstiprotokolla (MCP) -palvelimiin ja jopa graafisen käyttöliittymän automaatioon työpöytäsovelluksille tietokoneen käyttötyökalun avulla.

!!! tip
    Ajattele työkaluja "toimintablokkeina", jotka antavat agenttillesi supervoimia tehdä asioita _keskustelun_ ulkopuolella, kuten kutsua API:ta, suorittaa prosessi tai kerätä käyttäjän syötteitä automaattisesti.

#### Generatiivinen vastaussolmu

- **Tarkoitus** - käyttää suurta kielimallia tuottamaan luonnollisia, ihmismäisiä vastauksia käyttäjän kysymyksen ja mahdollisen yhdistetyn datan perusteella.
- **Esimerkki** - käyttää yhdistettyä tietolähdettä, joka sisältää tietoa lomaoikeuksista, vastatakseen käyttäjän kysymyksiin lomapyynnöistä.

Tämä solmu mahdollistaa agenttisi vastaamaan käyttäjän kysymyksiin käyttämällä tietoa eri tietolähteistä, kuten verkkosivustoista, dokumenteista, SharePointista tai mukautetusta datasta. Sitä voidaan käyttää varajärjestelmänä, kun sopivaa aihetta ei löydy, tai aiheen sisällä antamaan yksityiskohtaisempia, dynaamisia vastauksia määrittelemiesi lähteiden perusteella.

!!! tip
    Ajattele tätä "älykäs vastausblokki", joka auttaa agenttiasi antamaan hyödyllisiä, tarkkoja vastauksia etsimällä luotettavaa sisältöä, jonka sinä määrität.
...
- **Esimerkki** - kun käyttäjä kysyy lomapäivien saldoaan, agentti suorittaa `GET`-pyynnön lomahallintajärjestelmään ja poimii `remainingLeaveDays` API-vastauksesta ja vastaa käyttäjälle arvolla.

Tämä solmu antaa agenttisi yhdistää ulkoisiin järjestelmiin lähettämällä REST API-kutsuja, kuten `GET` tai `POST`-pyyntöjä. Voit mukauttaa pyyntöä otsikoilla, sisällöllä ja jopa käyttää Power Fx:ää sisällyttämään dynaamisia tietoja, ja tallentaa vastauksen muuttujiksi myöhempää käyttöä varten keskustelussa.

!!! tip
    Ajattele tätä "ota yhteyttä ja hanki tietoa" -blokiksi, joka auttaa agenttiasi kommunikoimaan muiden palveluiden kanssa, kuten käyttäjätietojen hakeminen tai tietojen lähettäminen toiseen järjestelmään.
...
Tämä solmu antaa agenttisi lähettää ei-viestitoimintoja ulkoisiin järjestelmiin tai kanaviin, jotka voivat sitten päättää, miten vastata. Annat jokaiselle tapahtumalle nimen ja liität arvon, joka voi olla yksinkertainen numero tai teksti, muuttuja tai Power Fx-kaava, ja se lähetetään JSON-objektina.
...
Copilot Studiossa Power Fx on matalan koodin ohjelmointikieli, jota käytetään lisäämään logiikkaa ja dynaamista käyttäytymistä agenttiisi. Se on sama kieli, jota käytetään Microsoft Power Appsissa, ja se on suunniteltu yksinkertaiseksi ja Excel-tyyliseksi, mikä tekee siitä helppokäyttöisen sekä kehittäjille että ei-kehittäjille.
...
- **Tehokas:** sen avulla voit personoida keskusteluja, validoida syötteitä ja hallita, miten agenttisi käyttäytyy käyttäjädatan perusteella.
...
Tämä antaa sinun rakentaa räätälöityjä keskustelun kulkuja alusta alkaen, ja voit lisätä tai poistaa solmuja tarpeen mukaan
Tämä mahdollistaa sen, että voit kuvata haluamasi luonnollisella kielellä, ja Copilot rakentaa keskustelun puolestasi. Sama pätee aiheen muokkaamiseen; käytä luonnollista kieltä, ja Copilot tarkistaa ja muokkaa aiheen puolestasi.
...
- Helpottaa keskustelun kulkujen iterointia ja parantamista vähäisellä vaivalla.

#### ✨ Esimerkkikehotteet

- **Luo aihe**
      - `Hyväksy käyttäjän nimi, ikä ja syntymäaika ja toista sitten heidän vastauksensa takaisin heille`
      - `Kerää käyttäjän katuosoite, osavaltio ja postinumero. Käyttäjän tulisi voida yrittää jokaista kysymystä uudelleen enintään 4 kertaa`

- **Muokkaa aihetta**
      - `Lisää kysymys käyttäjän syntymäajan kysymiseksi`
      - `Yhteenveto kerätystä tiedosta Adaptive Card -kortissa.`

## 👩🏻‍🎨 OK, miten suunnittelen aiheita agentilleni?
...
- Kysymyksiä, joita käyttäjät kysyvät usein, kuten `mitkä ovat oikeuteni sairauslomapäiviin?`
- Yleisiä tehtäviä, joita käyttäjät haluavat suorittaa, kuten lomakkeen lähettäminen
- Ongelmia, joita käyttäjät kohtaavat usein, kuten kirjautumisongelmat
...
- Vianmääritys - käyttäjä tarvitsee apua ongelman ratkaisemiseksi
...
!!! tip
    Pidä keskustelu lyhyenä ja keskittyneenä. Kysy vain tarpeelliset asiat.
...
- **Monivaiheinen** - edestakainen keskustelu jatkokysymyksillä

Esimerkki:

- Käyttäjä: `Haluan hakea lomaa.`

- Agentti: `Selvä! Milloin haluaisit loman alkavan?`
...
Vaikka olet suunnitellut aiheen lomapyyntöjä varten, käyttäjät voivat kysyä kysymyksiä, jotka eivät suoraan kuulu aiheeseen. Tässä AI-ominaisuudet, kuten _Keskustelun tehostaminen_ -järjestelmäaihe, ovat hyödyllisiä.

Tämä aihe sisältää generatiivisen vastaussolmun, joka mahdollistaa agentin käyttämään yhdistettyjä tietolähteitä heti. Kaikki agenttitasolla lisätyt tietolähteet sisältyvät automaattisesti generatiiviseen vastaussolmuun _Keskustelun tehostaminen_ -järjestelmäaiheessa.
...
- Sinun ei tarvitse luoda aihetta manuaalisesti jokaista politiikkaan liittyvää kysymystä varten.
...
- Testaa käyttämällä oikeita kysymyksiä tai oikeita näytteitä.
- Varmista, että se kuulostaa luonnolliselta ja hyödylliseltä.

!!! tip
    Tee parannuksia aiheeseen testauksen perusteella, kuten lisäämällä solmuja tai poistamalla solmuja ja ohjaamalla toiseen aiheeseen.

### ⚠️ Vaihe 7 - Vältä verkkosivuston sisällön kopioimista

Älä kopioi sisältöä, joka on jo verkkosivustollasi.

- Keskity aiheisiin, joita käyttäjät kysyvät usein.
...
#### Vaihe 2: Agentti kysyy tietoja Adaptive Card -kortilla

Agentti kysyy,

`Selvä! Mitkä päivämäärät haluaisit ottaa vapaaksi?`

Adaptive Card -kortissa on aloituspäivä ja lopetuspäivä kalenterivalintakontrolli.

#### Vaihe 3: Käyttäjä antaa päivämäärät

Käyttäjä valitsee aloituspäiväksi 5. elokuuta 2025 ja lopetuspäiväksi 10. elokuuta 2025 ja lähettää kortin. Päivämääräarvot tallennetaan Adaptive Card -kortin tuloksiin muuttujina.

#### Vaihe 4: Pilvitoiminto suoritetaan

Power Automate -pilvitoiminto suoritetaan, joka luo uuden pyynnön lomahallintajärjestelmään ja lähettää sähköpostin esimiehelle lomapyynnöstä.

#### Vaihe 5: Lähetä vahvistusviesti käyttäjälle

Agentti vastaa,

`Lomapyyntösi 5.–10. elokuuta on lähetetty. Esimiehesi tarkistaa sen ja palaa asiaan pian.`

## 🧪 Lab 07 - Lisää uusi aihe keskustelusolmuilla

Nyt opimme, miten lisätään uusi aihe laukaisimella ja työkaluilla. Tämä labra kattaa aiheen luomisen tyhjästä, jotta ymmärrät, miten mukauttaa aiheita tarpeisiisi.

- [7.1 Lisää uusi aihe tyhjästä](../../../../../docs/recruit/07-add-new-topic-with-trigger)
- [7.2 Määritä laukaisun syötteet ja tulosteet](../../../../../docs/recruit/07-add-new-topic-with-trigger)
...
1. **SharePoint-lista**
    ...
    Käytämme **Laitteet** SharePoint-listaa [Lesson 00 - Kurssin asennus - Vaihe 3: Luo uusi SharePoint-sivusto](../00-course-setup/README.md#step-4-create-new-sharepoint-site).

    Jos et ole vielä luonut **Laitteet** SharePoint-listaa, palaa [Lesson 00 - Kurssin asennus - Vaihe 3: Luo uusi SharePoint-sivusto](../00-course-setup/README.md#step-4-create-new-sharepoint-site).

1. **Contoso Helpdesk Agent**

    Käytämme aiemmin luotua agenttia [Lesson 06 - Luo mukautettu agentti luonnollisella kielellä Copilotin avulla ja yhdistä se tietoihisi](../06-create-agent-from-conversation/README.md).

### 7.1 Lisää uusi aihe tyhjästä

1. Valitse **Aiheiden välilehti** agentin nimen vierestä. Jos et näe sitä, valitse **+6**, jolloin näet **Aiheet**-kohdan.

    ![Valitse Aiheet](../../../../../translated_images/7.1_01_Topics.789ffa4f75830b5f25fb1eeb8fa3e8ba3810b95870cb3dc49d80d8f4ba15a00a.fi.png)

1. **Aiheet**-välilehti latautuu, ja oletuksena näytetään _Mukautetut_ aiheet. Voit suodattaa aiheita Kaikki, Mukautetut ja Järjestelmä. Mukautetut ja järjestelmäaiheet, jotka näet tällä hetkellä, luotiin automaattisesti, kun agentti otettiin käyttöön.

    Valitse **+ Lisää aihe** ja valitse **Tyhjästä**.
...
1. Anna laukaisukuvaukselle selitys, joka kuvaa, mitä aihe tekee. Kopioi ja liitä seuraava.

    ```text
    This topic helps users find devices that are available from our SharePoint Devices list. User can ask to see available devices and it will return a list of devices that are available which can include laptops, smartphones, accessories and more.
    ```

    ![Anna nimi ja kuvaus laukaisulle](../../../../../translated_images/7.1_04_TriggerDescription.cb748c0358b3af249fcc0fdb0b262185ffed14d8cf628186b8a65ad4bbf14172.fi.png)

### 7.2 Määritä laukaisun syötteet ja tulosteet

1. Seuraavaksi lisäämme uuden syöttömuuttujan, jota generatiivinen tekoäly käyttää orkesteroinnissaan laitetyypin arvon poimimiseen käyttäjän viestistä. Valitse aiheen **Lisää ellipsi (...)** ja valitse **Tiedot**, jotta näet aiheen tietopaneelin.

    ![Valitse aiheen tiedot](../../../../../translated_images/7.2_01_SelectTopicDetails.cc1b97a86718e101084c366514cc306fe82243a014a44c579394e0f70ba5ca53.fi.png)

1. **Aiheen tiedot**-paneeli on nyt latautunut. Valitse **Syöte**-välilehti.

    ![Syöte-välilehti](../../../../../translated_images/7.2_02_SelectInputTab.d0b900eb02a8f982324f59e3b7aca34c2cdba78263acdc9301225e1c3e6338b6.fi.png)

1. Valitse **Luo uusi muuttuja**.
...
1. Nyt meidän täytyy määrittää syöttö- ja tulostemuuttujamme. Seuraavat ovat ominaisuuksia, jotka voidaan määrittää aiheen syötteille ja tulosteille.
    ...
    | Tunnista nimellä  | Entiteettityyppi, jonka avulla agentti voi tunnistaa oikean arvotyypin  |
...
1. Seuraavaksi määritetään aiheen tuloste. Valitse **Tuloste**-välilehti.

    ![Valitse Tuloste-välilehti](../../../../../translated_images/7.2_07_SelectOutputTab.ab5aa0a2f385f1492df5a67f8f2cbed752dc0258c1e1ddb9928d204405ec403a.fi.png)
...
1. Päivitä seuraavat ominaisuudet.
    ...
    **Muuttujan tietotyyppi** - Valitse **Taulukko** tietotyypiksi.
...
1. Olemme nyt määrittäneet aiheen syötteet ja tulosteet. Valitse **X-kuvake** poistuaksesi **Aiheen tiedot**-paneelista.

    ![Poistu aiheen tietopaneelista.](../../../../../translated_images/7.2_10_ExitTopicDetailsPane.6e8981434f04049bef7ab93f9545ee433087e1c99cdfe27b355ee9858ddfde99.fi.png)

### 7.3 Lisää työkalu liittimen avulla

1. Seuraavaksi lisätään solmu, joka mahdollistaa agentin hakea laitelistauksen **Laitteet** SharePoint-listasta. Valitse **+ kuvake** laukaisun alapuolella.

    ![Lisää työkalu](../../../../../translated_images/7.3_01_AddNode.4656328835f7a28bc5f66c440d620a0990bf098e858619ff2c32a9b14fae7c4f.fi.png)

1. Valitse **Lisää työkalu**-solmu ja sitten **Liitin**-välilehti. Etsi `Get items` ja valitse **Get items** SharePoint-liittimen toiminto.
...
1. Liittimelle täytyy luoda uusi yhteys. Valitse **chevron**-kuvake ja valitse **Luo uusi yhteys**.
...
1. Näytetään kaksi vaihtoehtoa, joiden avulla voit yhdistää suoraan SharePoint Onlineen tai paikalliseen SharePointiin. Oletuksena **Yhdistä suoraan (pilvipalvelut)**-vaihtoehto on valittuna, ja käytämme sitä yhteyteemme.
Valitse **Luo**.
...
1. Seuraavaksi sinun täytyy vahvistaa, että käyttäjätiliäsi voidaan käyttää yhteyden muodostamiseen SharePoint-liittimeen. Valitse **Salli pääsy**.
...
1. Valitse **Lähetä**, jotta **Get items** SharePoint-liittimen toiminto lisätään solmuksi aiheeseen.
...
1. **Get items** SharePoint-liittimen toiminto on nyt lisätty aiheeseen. Voimme nyt aloittaa toiminnon syötteiden määrittämisen. Valitse **kolmen pisteen (...) kuvake** ja valitse **Ominaisuudet**.
...
1. **Get items** -määrittelypaneeli avautuu, ja oletuksena näet **Syötteet**-välilehden. Valitse **Aloitus**-välilehti ja kirjoita kuvaus **Käyttökuvaus**-kenttään. Kopioi ja liitä seuraava.
...
![Get items -kuvaus](../../../../../translated_images/7.3_09_UpdateDescription.76a8d2ebddd4c3e4ca423daad7485afa60f31f37c97e16529d94e224f9709d60.fi.png)

1. Valitse **Syötteet**-välilehti ja valitse **Contoso IT** -sivusto ja **Devices**-lista, jonka loit [Oppitunti 00 - Kurssin valmistelu - Vaihe 3: Luo uusi SharePoint-sivusto](../00-course-setup/README.md#step-4-create-new-sharepoint-site).

![Määritä Get items -syötteet](../../../../../translated_images/7.3_10_GetItemsInputs.17f8689e660c480dd405ab2ab57db34dcd2b8e697ec09d54c8f8649eb619c58b.fi.png)

1. Nyt, jotta näytetään vain laitteet SharePoint-listasta seuraavien ehtojen perusteella:
   - valittu arvo,
...
1. Valitse **laajennus**-kuvake suurentaaksesi **Kaava**-kenttää. Kopioi ja liitä seuraava Power Fx -lauseke.

Käytämme `Concatenate`-funktiota luodaksemme lausekkeen, joka suodattaa
- SharePoint-sarakkeen **Tila** arvoksi _Saatavilla_
- ja SharePoint-sarakkeen **Omaisuuden tyyppi** arvoksi _valittu laite kysymyssolmusta_.
...
1. Power Fx -lauseke sovelletaan nyt **Get items** -toiminnon Suodatuskysely-syöteparametriin. Seuraavaksi valitse **Kaikki kohteet** -näkymä **Rajoita sarakkeet näkymän mukaan** -kohdassa. Tämä hakee vain listan sarakkeet valitun näkymän perusteella.
...
1. Käytämme nyt PowerFx-lauseketta asettaaksemme muuttujan arvoksi `value`-ominaisuuden, joka palautetaan **Get items** -vastauksessa, ja teemme [muuttujan laajuuden](https://learn.microsoft.com/microsoft-copilot-studio/advanced-power-fx?WT.mc_id=power-172618-ebenitez) globaaliksi lisäämällä etuliitteen `Global`.
...
Tämä ohje opastaa generatiivista tekoälyä käyttämään **Saatavilla olevat laitteet** -laukaisinta näyttääkseen listan saatavilla olevista laitteista **Devices** SharePoint-listasta. Valitse koko aiheen paikkamerkki hakasuluissa.
...
1. Agentti suorittaa SharePoint-työkalun, joka hakee suodatetun listan laitteista, joissa laitetyyppi on "kannettava tietokone" ja tila on "saatavilla", käyttäen Power Fx -lauseketta. Vastaus palautetaan käyttäjälle luettelomuodossa.
...
1. Täällä näemme Get items -toiminnon yksityiskohdat ja muistamme aiemmin syötetyn _käyttökuvauksen_? Täällä näemme käyttökuvauksen. Valitse **Sulje**.

> Tämä kertoo meille, mitä toimintoja käytetään ja niiden tarkoituksen. Tämä pitää yhteytemme järjestyksessä 📁.
...
Onnittelut! 👏🏻 Olet oppinut, kuinka lisätä uusi aihe alusta alkaen, kuinka lisätä työkalu, joka kutsuu Get items SharePoint-liittimen toiminnon, ja käyttää Power Fx:ää suodattamaan vastausta niin, että palautetaan vain laitteet, joiden tila on saatavilla ja laitetyyppi on kannettava tietokone. 🙌🏻
...
⏭️ [Siirry oppituntiin **Paranna käyttäjävuorovaikutusta mukautuvilla korteilla**](../08-add-adaptive-card/README.md)
